...
impl PartialEq for PageList {
    fn eq(&self, other: &Self) -> bool {
        if self.wiki().unwrap_or_default() != other.wiki().unwrap_or_default() {
            return false;
        }
...
impl PageList {
    ...
    pub fn clear(&mut self) -> Result<()> {
        *self.wiki.write().map_err(|e| anyhow!("{e}"))? = None;
...
WHERE {} IN ({})",prefix,&field_name,namespace_id,table,term_in_lang_id,&field_name,item_ids);
    ...
    pub fn to_pagelist_disk(&self) -> Result<PageListDisk> {
        ...
        for entry in entries.iter() {
            pagelist.add_entry(entry.to_owned())?;
        }
        Ok(pagelist)
    }
}
