...
function setPermalink() {
	...
	if (psid != 0) {
		...
		h += psid_note;
		h +=
			" <span><a target='_blank' href='https://fist.toolforge.org/wdfist/index.html?psid=" +
			psid +
			"&no_images_only=1&remove_used=1&remove_multiple=1&prefilled=1'>";
...
function applyParameters() {
	...
	if (
		params["doit"] != "" &&
		typeof params["referrer_url"] != "undefined" &&
		params["referrer_url"] != "" &&
		params["psid"] != ""
	) {
		var psid = 0;
...
function setInterfaceLanguage(l) {
	...
	function specialUpdates() {
		...
		$('a[tt="manual"]').attr({
			href: "https://meta.wikimedia.org/wiki/PetScan/" + l,
		});
		$("#query_length").text(
			_t("query_length").replace("$1", $("#query_length").attr("sec")),
		);
		$("#num_results").text(
			_t("num_results").replace("$1", $("#num_results").attr("num")),
		);
...
function loadInterface(init_lang, callback) {
	loadNamespaces(function () {
		...
		$('input[name="language"]').keyup(function () {
			loadNamespaces();
		});
		$('input[name="project"]').keyup(function () {
			loadNamespaces();
		});
...
function loadNamespaces(callback) {
	...
	function namespaceDataLoaded(d) {
		...
		$("#namespaces input").change(function () {
			var o = $(this);
			var ns = o.attr("ns") * 1;
			namespaces_selected = jQuery.grep(namespaces_selected, function (value) {
				return value != ns;
			}); // Remove is present
...
function wdq2sparql(wdq, sparql_selector) {
	...
	$.get(
		"/wdq2sparql/?" + encodeURIComponent(wdq),
		function (d) {
			...
			sparql = sparql.replace(/^prefix.+$/gim, "");
...
function generateThumbnailView() {
	...
	$("#main_table tbody tr").each(function () {
		...
		var url =
			a
				.attr("href")
				.replace(/\/wiki\/File(:|%3[aA])/, "/wiki/Special:Redirect/file/") +
			"?width=" +
			thumbnail_size;
		var h = "";
		h += '<div class="card">';
		h +=
			'<div style="text-align:center"><a class="thumblink"><img class="card-img-top pre_thumb" src2load="' +
			url +
			'" alt="' +
			"??" +
			'" border=0 /></a></div>';
...
function validateSourceCombination() {
	...
	var text = $.trim(o.val());
	var reg =
		/^( and | or | not |categories|sparql|pagepile|manual|wikidata|search|\(|\)| )*$/gi;
...
function initializeInterface() {
	...
	if (typeof p.cats != "undefined" && (p.categories || "") == "")
		p.categories = p.cats;
	...
	function highlightMissingWiki(n1, n2) {
		...
		if ($.trim(text) != "" && !wiki.match(/(wiki|source|quote)\s*$/)) {
			//$.trim(wiki) == '' ) {
	...
	$('[name="manual_list"]').keyup(function () {
	...
	$('[name="manual_list_wiki"]').keyup(function () {
		highlightMissingWiki("manual_list", "manual_list_wiki");
	});
	$('[name="search_wiki"]').keyup(function () {
		highlightMissingWiki("search_query", "search_wiki");
	});
	$('[name="common_wiki_other"]').keyup(function () {
		highlightMissingWiki("common_wiki_other", "common_wiki_other");
	});
	highlightMissingWiki("manual_list", "manual_list_wiki");
	highlightMissingWiki("search_query", "search_wiki");
	...
	$("#tab-list").click(function () {
		if ($("#main_form div.tab-pane").length > 0) {
			...
			$("#main_form div.tab-pane")
				.addClass("former-tab-pane")
				.removeClass("tab-pane");
		} else {
			$("#tab-list").text(_t("toggle_list"));
			$("#main_form ul.nav-tabs").show();
			$("#main_form div.former-tab-pane")
				.addClass("tab-pane")
				.removeClass("former-tab-pane");
		}
	});
	...
	$("#quick_wikispecies").click(function () {
		$("input[name='language']").val("species");
		$("input[name='project']").val("wikimedia");
		loadNamespaces();
		return false;
	});
	$("#quick_wikidata").click(function () {
		$("input[name='language']").val("wikidata");
		$("input[name='project']").val("wikimedia");
		loadNamespaces();
		return false;
	});
...
$(document).ready(function () {
	...
	fetch("https://ores.wikimedia.org/v3/scores/")
