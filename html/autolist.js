function WiDaR(callback) {
  ...
  this.getInfo = function () {
    ...
    this.abstractCall(
      ...
      function (d) {
        ...
        me.userinfo = {};
        if (
          typeof (((d || {}).result || {}).query || {}).userinfo == "undefined"
        ) {
          callback();
          return;
        }
  ...
  this.getLoginLink = function (text) {
    var h =
      ...
      this.server +
      "/widar/index.php?action=authorize'>" +
      text +
      "</a>";
    return h;
  };
...
function AutoList(callback) {
  ...
  this.runCommand = function (id) {
    ...
    if (cmd.mode == "create") {
      ...
      $.getJSON(
        "https://" +
          autolist_wiki_server +
          "/w/api.php?action=wbgetentities&ids=Q" +
          cmd.q +
          "&format=json&callback=?",
        function (d) {
          var done = false;
          $.each(
            (((d.entities || {})["Q" + cmd.q] || {}).claims || {})[
              cmd.prop.toUpperCase()
            ] || {},
            function (k, v) {
  ...
  this.allDone = function () {
    ...
    $("#al_start_qs").show();
    $("#al_status").html(
      "<span style='color:green'>" + _t("al_done") + "</span>",
    );
  };
  ...
  this.runNextCommand = function () {
    ...
    if (me.running.length >= me.concurrent) {
      setTimeout(function () {
        me.runNextCommand;
      }, 100); // Was already called, so short delay
      return;
    }
  ...
  this.is_wikidata = function () {
    return (
      typeof autolist_wiki_server == "undefined" ||
      autolist_wiki_server == "" ||
  ...
  this.initializeAutoListBox = function () {
    ...
    h += "<div class='autolist_subbox'>";
    h +=
      "<button id='al_do_check_all' class='btn btn-outline-secondary btn-sm' tt='al_all' style='width:100%'></button><br/>";
    h +=
      "<button id='al_do_check_none' class='btn btn-outline-secondary btn-sm' tt='al_none' style='width:100%'></button><br/>";
    h +=
      "<button id='al_do_check_toggle' class='btn btn-outline-secondary btn-sm' tt='al_toggle' style='width:100%'></button><br/>";
    h += "</div>";
    h += "<div class='autolist_subbox'>";
    h +=
      "<textarea id='al_commands' tt_placeholder='al_commands_ph' rows=3 style='padding:2px;width:200px'>" +
      (p.statementlist || "") +
      "</textarea><br/>";
    ...
    h +=
      "<button id='al_start_qs' class='btn btn-outline-success btn-sm' tt='al_start_qs'></button>";
    h +=
      "<button id='al_do_stop' class='btn btn-outline-danger btn-sm' tt='al_stop' style='display:none'></button>";
    h +=
      "<form style='display:none' id='qs_form' action='//tools.wmflabs.org/quickstatements/api.php' method='post' target='_blank'>";
    ...
    h +=
      ...
      "' />";
    h +=
      "<textarea type='hidden' id='qs_commands' name='data'></textarea><button name='yup'></button></form>";
    ...
    $("#bot_concurrent").keyup(function () {
      updateConcurrency();
    });
    $("#bot_concurrent").change(function () {
      updateConcurrency();
    });
    ...
    $("#al_start_qs").click(function (e) {
      ...
      let qs_commands = [];
      let entity_letter =
        autolist_wiki_server == "www.wikidata.org" ? "Q" : "M";
      $.each(me.commands_todo, function (dummy, cmd) {
        ...
        if (cmd.mode == "create") {
          ...
          let the_label = $.trim(
            cmd.page.replace(/_/g, " ").replace(/\s*\(.+?\)\s*/, " "),
          ).replace(/^\S+:/, "");
    ...
    $("#al_do_stop").click(function (e) {
      ...
      $("#al_do_stop").hide();
      $("#al_status").html(
        "<span style='color:red;'>" + _t("al_stopped") + "</span>",
      );
    });
  ...
  this.addCheckLinks = function () {
    ...
    $("#main_table tbody tr").each(function () {
      ...
      var page = $(td.find("a").get(0))
        ...
        .replace(/_/, " ");
      td.append(
        " <span class='pull-xs-right smaller'>[<a tt='check_wd' target='_blank'></a>]</span>",
      );
      tt.updateInterface(td);
      td.find('a[tt="check_wd"]').attr({
        href:
          "https://tools.wmflabs.org/wikidata-todo/duplicity.php?wiki=" +
          output_wiki +
          "&norand=1&page=" +
          page,
      });
